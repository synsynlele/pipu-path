-- Stage 11 safeguarding hardening for eligibility changes after a request is created.
...
begin
  ...
  if not private.stage11_builder_connect_eligible(other_user) then
    raise exception 'CONNECT_ADULT_REQUIRED' using errcode='P0001';
  end if;
...
begin
  ...
  if not private.stage11_builder_connect_eligible(target_user_id_input)
     and not exists (
