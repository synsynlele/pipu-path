-- Allow a Builder to change a selected Possible Path without erasing truthful
-- developmental history. A changed Path closes only unfinished current work;
-- completed Quests, evidence, reflections, XP and completed Projects remain.
...
begin
  ...
  -- A first selection does not rewrite pre-existing legacy state. Retirement is
  -- required only when the Builder deliberately pivots away from a selected Path.
  if recommendation.selected_path_key is not null then
    -- Freeze every unfinished Quest that belongs to the current non-terminal
    -- Mission/Journey lineage. Evidence/reflection rows and timestamps are not
