...
describe("safe Economic Path switching", () => {
  it("uses one authenticated atomic RPC and validates ownership and the requested path", () => {
    expect(migration).toContain("create or replace function public.switch_economic_path");
    expect(migration).toContain("security definer");
    expect(migration).toContain("actor uuid := auth.uid()");
    expect(migration).toContain("and user_id = actor");
    expect(migration).toContain("for update");
    expect(migration).toContain("jsonb_array_elements(recommendation.possible_paths)");
  ...
  it("retires only unfinished current work and preserves completed developmental history", () => {
    ...
    expect(migration).toContain("replaced_at = coalesce(journey.replaced_at, now())");
    expect(migration).toContain("mission.status in ('draft', 'active', 'paused')");
    expect(migration).not.toMatch(/delete\s+from\s+public\.(user_quests|user_journeys|user_missions|quest_evidence|quest_reflections|builder_xp_transactions)/i);
    expect(migration).not.toMatch(/update\s+public\.(quest_evidence|quest_reflections|builder_xp_transactions)/i);
  });

  it("does not retire prior work on an initial Path selection", () => {
    expect(migration).toContain("if recommendation.selected_path_key is not null then");
    expect(migration).toContain("if recommendation.selected_path_key = path_key_input then");
  });
  ...
  it("requires explicit confirmation and tells the Builder what is preserved", () => {
    ...
    expect(form).toContain(
      "Completed work, proof, reflections, XP\n            and Projects stay saved.",
    );
  });
});
