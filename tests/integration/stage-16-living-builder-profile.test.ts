...
const migration = read("supabase/migrations/20260817191000_stage_16_living_builder_profile.sql");
...
describe("Stage 16 Living Builder Profile structure", () => {
  it("stores versioned private profile claims, evidence and Builder feedback", () => {
    ...
    expect(migration).toContain("create table public.builder_capability_claims");
    expect(migration).toContain("create table public.builder_capability_evidence");
    expect(migration).toContain("create table public.builder_capability_feedback");
    expect(migration).toContain("enable row level security");
    expect(migration).toContain("revoke all on public.builder_profile_versions");
  ...
  it("derives capability evidence only from completed proof-bearing action", () => {
    ...
    expect(migration).toContain("contribution.contributor_id = collaboration.owner_id");
    expect(migration).toContain("contribution.contributor_id = collaboration.collaborator_id");
  });

  it("uses explainable conservative progression rules and exact evidence links", () => {
    expect(migration).toContain("sum(evidence.strength) >= 4 and count(*) >= 2");
    ...
    expect(migration).toContain("'/connect/collaborations/' || collaboration.id::text");
  });
