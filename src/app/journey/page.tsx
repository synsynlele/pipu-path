...
function JourneyMap({
  ...
  return (
    <div className="mt-7">
      ...
      <div
        className="mt-5 [scrollbar-width:thin] overflow-x-auto pb-3"
        ...
        <ol className="flex min-w-max items-stretch gap-0 pr-5">
          {journey.milestones.map((milestone, index) => {
            ...
            return (
              <li
                key={milestone.id}
                className="relative flex w-48 shrink-0 flex-col sm:w-56"
              >
                ...
                <div className="mt-3 pr-5">
                  <p
                    className={`text-[0.68rem] font-semibold tracking-wide uppercase ${current ? "text-primary" : completed ? "text-success" : "text-muted"}`}
                  >
...
function JourneyDetails({
  ...
  return (
    <>
      ...
      <details className="border-border open:bg-background/40 mt-5 rounded-2xl border p-4">
          ...
          <ol className="grid gap-3">
            {journey.milestones.map((milestone) => (
              ...
              >
                ...
                <h4 className="text-navy mt-2 font-semibold">
                  {milestone.title}
                </h4>
                <p className="text-muted mt-2 text-sm leading-6">
                  {milestone.purpose}
  ...
  return (
    ...
    >
      <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[#07142f] p-6 text-white sm:p-9">
        ...
        <div
          aria-hidden="true"
          className="absolute right-12 -bottom-32 size-72 rounded-full bg-[#4f7cff]/18 blur-3xl"
        />
      ...
      {!context ? (
        <Surface className="mt-6 p-6 sm:p-8">
          ...
          <h2 className="text-navy mt-2 text-2xl font-semibold">
            Choose the Campaign first.
          </h2>
          <p className="text-muted mt-3 max-w-2xl text-sm leading-6">
            Your Mission gives the Journey a destination. Activate one practical
            direction before opening the map.
          </p>
          <ButtonLink href="/mission" className="mt-5">
            Open my Mission
          </ButtonLink>
        </Surface>
      ) : state.active ? (
        ...
        <Surface className="mt-6 p-5 sm:p-8">
          ...
          {state.continuationAvailable ? (
            <div className="border-gold/30 bg-gold/5 mt-6 rounded-2xl border p-5">
              ...
              <h3 className="text-navy mt-2 text-xl font-semibold">
                Open growth cycle {state.nextCycleNumber}
              </h3>
              <p className="text-muted mt-2 text-sm leading-6">
                The next Journey builds on what actually happened here. It does
                not erase your previous evidence.
              </p>
              <div className="mt-4">
                <JourneyGenerationForm
                  kind="continue"
            ...
            <div className="border-border mt-6 rounded-2xl border p-5">
              <p className="text-primary text-xs font-semibold tracking-wide uppercase">
                One major Build remains
              </p>
              <h3 className="text-navy mt-2 text-xl font-semibold">
                Turn this Journey into something useful.
              </h3>
