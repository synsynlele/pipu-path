    ...
    >
      <section className="border-gold/20 bg-panel rounded-[2rem] border px-6 py-10 sm:px-10 sm:py-14">
        ...
        <p className="text-muted mt-5 max-w-3xl text-lg leading-8">
          PipuPath can prove that evidence exists. A collaborator can confirm
          only what they personally observed in the exact completed
          collaboration you shared.
        </p>
        <div className="mt-7 flex flex-wrap gap-3">
          <ButtonLink href="/profile" variant="secondary">
            Back to Living Profile
          </ButtonLink>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <div>
            <h2 className="text-lg font-semibold">
              {capabilityVerificationTrustCopy.systemEvidence}
            </h2>
            <p className="text-muted mt-2 leading-7">
              Your Living Builder Profile already records completed action
              evidence and its source.
            </p>
          </div>
          <div>
            <h2 className="text-lg font-semibold">
              {capabilityVerificationTrustCopy.humanConfirmation}
            </h2>
            <p className="text-muted mt-2 leading-7">
              {capabilityVerificationTrustCopy.boundary}
            </p>
          </div>
        </div>
        <p className="text-muted mt-4 text-sm leading-6">
          No stars, endorsements, popularity counts or paid verification.
          Nothing here becomes public automatically.
        </p>
      </Surface>
      ...
      <section className="mt-10" aria-labelledby="verified-heading">
        ...
        >
          Human confirmation you have actually earned.
        </h2>
        {workspace.verifiedCapabilities.length === 0 ? (
          <Surface className="mt-5 p-6">
            <p className="text-muted leading-7">
              No collaborator-confirmed capability yet. Complete collaborative
              work first, then request confirmation from the person who built
              with you.
            </p>
          </Surface>
        ) : (
            ...
            {workspace.verifiedCapabilities.map((item) => (
              <Surface key={item.capabilityKey} className="p-5">
                ...
                <p className="text-muted mt-2 text-sm">
                  {item.confirmedCount} evidence-bound confirmation
                  {item.confirmedCount === 1 ? "" : "s"}
                </p>
              </Surface>
            ))}
          </div>
      ...
      <section className="mt-12" aria-labelledby="request-heading">
        ...
        <h2
          ...
          className="mt-3 text-3xl font-semibold tracking-tight"
        >
          Only completed collaboration evidence is eligible.
        </h2>
        {workspace.eligibleEvidence.length === 0 ? (
          <Surface className="mt-5 p-6">
            <p className="text-muted leading-7">
              There is no eligible unverified collaboration evidence right now.
              Finish a structured collaboration, refresh your Living Builder
              Profile, and return here.
            </p>
            <ButtonLink href="/connect/collaborations" className="mt-4">
              Open collaborations
            </ButtonLink>
          </Surface>
        ) : (
            ...
            {workspace.eligibleEvidence.map((item) => (
              <Surface key={item.evidenceId} className="p-6">
                ...
                <p className="mt-4 text-sm">
                  ...
                  to confirm this capability from the work you completed
                  together.
                </p>
                <form
                  action={requestCapabilityVerificationAction}
                  className="mt-5"
                >
                  ...
                  />
                  <label
                    htmlFor={`request-${item.evidenceId}`}
                    className="text-sm font-semibold"
                  >
                    Optional context
                  </label>
                  <textarea
                    id={`request-${item.evidenceId}`}
                    name="requestNote"
                    rows={2}
                    maxLength={400}
                    placeholder="What should your collaborator confirm from this work?"
      ...
      <section className="mt-12" aria-labelledby="incoming-heading">
        ...
        <h2
          ...
          className="mt-3 text-3xl font-semibold tracking-tight"
        >
          Confirm only what you personally observed.
        </h2>
        {workspace.incoming.length === 0 ? (
          <Surface className="mt-5 p-6">
            <p className="text-muted">
              No verification requests are waiting for you.
            </p>
          </Surface>
        ) : (
            ...
            {workspace.incoming.map((item) => (
              <Surface key={item.id} className="p-6">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="text-gold text-xs font-semibold uppercase">
                      {capabilityVerificationStatusLabel(item.status)}
                    </p>
                    <h3 className="mt-2 text-xl font-semibold">
                      {item.capabilityLabel}
                    </h3>
                ...
                {item.actionable ? (
                  <form
                    action={respondCapabilityVerificationAction}
                    className="mt-5"
                  >
                    <input
                      type="hidden"
                    ...
                    <textarea
                      ...
                      rows={2}
                      maxLength={600}
                      placeholder="Add only what you directly observed."
                      className="border-border bg-background text-foreground mt-2 w-full rounded-xl border px-3 py-2 text-sm"
                    />
                  ...
                  <form
                    action={closeCapabilityVerificationAction}
                    className="mt-4"
                  >
                    <input
                      type="hidden"
      ...
      <section className="mt-12" aria-labelledby="history-heading">
        ...
        <h2
          ...
          className="mt-3 text-3xl font-semibold tracking-tight"
        >
          Verification history stays traceable.
        </h2>
        {workspace.outgoing.length === 0 ? (
          <Surface className="mt-5 p-6">
            <p className="text-muted">
              You have not requested any capability verification yet.
            </p>
          </Surface>
        ) : (
          <div className="mt-5 space-y-4">
              ...
              <Surface key={item.id} className="p-5">
                  ...
                  {item.status === "pending" ? (
                    ...
                    <form action={closeCapabilityVerificationAction}>
                      ...
                      <Button
                        ...
                        variant="ghost"
                      >
                        Remove from my verification record
                      </Button>
                    </form>
                  ) : null}
                </div>
                {item.responseNote ? (
                  <p className="text-muted mt-3 text-sm">
                    Verifier context: {item.responseNote}
                  </p>
                ) : null}
              </Surface>
            ))}
