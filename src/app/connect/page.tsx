...
const errors: Record<string, string> = {
  ...
  blocked:
    "That network action is unavailable because one account blocked the other.",
      ...
      <main id="main-content" className="mx-auto max-w-4xl px-5 py-12 sm:px-8">
        ...
        <Surface className="mt-8 p-6 sm:p-8">
          ...
          <p className="text-muted mt-4 leading-7">
            This Connect release is limited to adult Builders who completed
            onboarding and are not under safeguarding review. PipuPath does not
            expose minors in a directory or enable unrestricted private
            messaging.
          </p>
  ...
  return (
    ...
    >
      ...
      <p className="text-muted mt-5 max-w-3xl text-lg leading-8">
        Discovery is opt-in. There is no open messaging, and contact details
        appear only after an accepted connection and an explicit sharing choice.
      </p>
        ...
        <Surface className="p-6 sm:p-8">
          ...
          <form action={saveNetworkProfileAction} className="mt-6 grid gap-5">
            ...
            <p className="text-muted -mt-2 text-xs leading-5">
              These details stay private until you accept a connection and
              deliberately share a channel.
            </p>
            <label className="border-border bg-soft flex items-start gap-3 rounded-xl border p-4 text-sm leading-6">
              ...
              <span>
                ...
                Show only my name, active Mission and the Builder fields above
                to eligible adults.
              </span>
            </label>
        ...
        <Surface className="p-6 sm:p-8">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-3 text-2xl font-semibold">
                Search by name, Mission, interest or capability.
              </h2>
            </div>
        ...
        <Surface className="mt-8 p-6">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            {state.blocked.map((builder) => (
              <form
                key={builder.userId}
                action={builderSafetyAction}
                className="border-border bg-soft flex items-center gap-3 rounded-xl border p-3"
              >
  ...
  return (
    <label className="grid gap-2 text-sm font-semibold">
      ...
      <span className="text-muted text-xs font-normal">
        Separate entries with commas. Up to eight.
      </span>
    </label>
  );
}
...
function ConnectionCard({ item }: { item: AcceptedConnection }) {
  return (
    <article className="border-border bg-soft rounded-2xl border p-4">
      ...
      <p className="text-muted mt-1 text-sm">@{item.username}</p>
      {item.sharedEmail ? (
        <p className="mt-3 text-sm">Email: {item.sharedEmail}</p>
      ) : null}
      {item.sharedWhatsapp ? (
        <p className="mt-1 text-sm">WhatsApp: {item.sharedWhatsapp}</p>
      ) : null}
      <form
        action={shareContactAction}
        className="border-border mt-4 grid gap-3 border-t pt-4 text-sm"
      >
        <input type="hidden" name="connectionId" value={item.connectionId} />
        ...
        <label className="flex items-center gap-2">
          ...
          />{" "}
          Share my saved WhatsApp
        </label>
        <Button type="submit" variant="secondary">
          Update Contact Sharing
        </Button>
      </form>
      <form action={respondConnectionRequestAction} className="mt-3">
        <input type="hidden" name="connectionId" value={item.connectionId} />
        <Button name="action" value="remove" type="submit" variant="ghost">
          Remove Connection
        </Button>
      </form>
    </article>
