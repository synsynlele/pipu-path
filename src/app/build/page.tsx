...
function currentAction(state: HomeState) {
      ...
      label:
        state.quest.status === "evidence_submitted"
          ...
          : state.quest.status === "available"
            ? "Start Quest"
            : "Do Next Step",
      progress: state.quest.status === "evidence_submitted" ? 82 : state.quest.status === "active" ? 56 : 28,
    };
  }

  if (state.project?.id && state.project.status === "active") {
    const progress = state.projectProgress
      ? Math.round((state.projectProgress.completed / Math.max(1, state.projectProgress.total)) * 100)
      : 24;
    return {
  ...
  if (state.journey) {
    return {
      ...
      detail: "Your Journey is the path. Open it and move into the next real-world challenge.",
      href: "/journey",
      label: "Open Journey",
      progress: 18,
    };
      ...
      <section className="bg-[#201b59] px-4 pt-5 pb-12 text-white sm:px-6 sm:pt-8 lg:px-8">
        <div className="mx-auto max-w-5xl">
          <div className="flex items-center justify-between gap-4 px-1">
            <div>
              <h1 className="text-[2rem] font-bold tracking-[-0.04em] sm:text-4xl">Build</h1>
        ...
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          <section className="pp-mobile-section">
            <div className="flex items-center justify-between gap-3 px-1">
              ...
              <Link href="/projects" className="text-sm font-semibold text-[#5b3be0]">
                All Builds
              </Link>
            </div>
            ...
            <div className="pp-app-card mt-3 overflow-hidden px-4 sm:px-5">
              <BuildFlowRow
                number="1"
                title="Journey"
                detail={state.journey?.title ?? "Choose the direction worth testing."}
              ...
              <BuildFlowRow
                ...
                detail={state.quest?.title ?? "A practical challenge appears when your Journey is ready."}
                status={state.quest ? "current" : state.journey ? "next" : "waiting"}
                href={state.quest?.id ? `/quests/${state.quest.id}` : "/quests"}
              />
              <BuildFlowRow
                number="3"
                title="Evidence"
                detail={proofSaved ? "Your latest Quest proof is saved." : "Bring back honest proof of what you tried or made."}
              ...
              <BuildFlowRow
                number="4"
                title="Reflection"
                detail={proofSaved ? "Reflection is the next move for this Quest." : "Reflection unlocks after evidence is submitted."}
              ...
              <BuildFlowRow
                number="5"
                title="Project"
                detail={state.project?.title ?? "Turn repeated capability into something useful for the world."}
                status={state.project?.id ? "ready" : "waiting"}
                href={state.project?.id ? `/projects/${state.project.id}` : "/projects"}
              />
            </div>
          </section>
          ...
          <section className="pp-app-card pp-mobile-section mt-4 flex items-center gap-4 p-4 sm:p-5">
            ...
            <div className="min-w-0 flex-1">
              <h2 className="text-base font-bold text-[#25284a]">Evidence of Progress</h2>
              <p className="mt-1 text-sm leading-5 text-[#747b90]">
                Proof belongs to the Quest or Project that produced it. Private evidence stays private by default.
              </p>
            </div>
            <ButtonLink
              href={state.quest?.id ? `/quests/${state.quest.id}` : state.project?.id ? `/projects/${state.project.id}` : "/projects"}
              variant="secondary"
          ...
          <section className="pp-app-card pp-mobile-section mt-4 p-5 sm:p-6">
            <div className="flex items-start gap-3">
              <span className="grid size-11 shrink-0 place-items-center rounded-full bg-gradient-to-br from-[#ff7d63] to-[#6f49e9] text-xl text-white">✦</span>
              <div>
                <p className="text-xs font-bold tracking-[0.12em] text-[#6848dc] uppercase">Nortnspoil reflection</p>
                <h2 className="mt-2 text-xl font-bold text-[#25284a]">Turn what happened into learning.</h2>
                <p className="mt-2 text-sm leading-6 text-[#747b90]">
                  Reflection is contextual. It appears inside the work that generated the lesson instead of becoming another detached feature.
                </p>
              </div>
            </div>
          </section>
...
function BuildFlowRow({
  ...
  return (
    ...
    >
      <span className={`grid size-10 shrink-0 place-items-center rounded-full text-sm font-bold ${stateClass}`}>
        {status === "ready" ? "✓" : number}
      </span>
      <span className="min-w-0 flex-1">
        <span className="block text-sm font-bold text-[#2a2d4d]">{title}</span>
        <span className="mt-1 block line-clamp-2 text-xs leading-5 text-[#7b8298]">{detail}</span>
      </span>
