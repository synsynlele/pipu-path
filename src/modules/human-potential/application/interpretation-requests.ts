...
const safeMessages: Record<string, string> = {
  HPI_DISCOVERY_INCOMPLETE: "Complete Discovery before preparing interpretation.",
  ...
  HPI_REQUEST_ALREADY_EXISTS: "An interpretation request is already being prepared.",
};

function safeError(error: { message: string } | null): HpiApplicationResult<never> {
  const code = error?.message.match(/HPI_[A-Z_]+/)?.[0] ?? "HPI_INTERPRETATION_NOT_ALLOWED";
  return { ok: false, code, message: safeMessages[code] ?? "Interpretation could not be prepared." };
}
...
> {
  ...
  const { data, error } = await client.rpc("normalize_stage4_discovery_evidence");
  if (error) return safeError(error);
  return { ok: true, value: { normalizedCount: data, localEvidenceCount: localEvidence.length } };
}
...
> {
  ...
  const client = await createServerSupabaseClient();
  const { data, error } = await client.rpc("create_stage4_interpretation_request", {
