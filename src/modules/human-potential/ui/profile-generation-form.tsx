...
export function ProfileGenerationForm() {
  const [state, action, pending] = useActionState(generateProfileAction, initialState);
  return (
    <form action={action} aria-busy={pending}>
      <Button type="submit" disabled={pending}>
        {pending ? "PipuPath is analysing your Discovery responses…" : "Generate my profile"}
      </Button>
      {pending ? (
