...
export const aggregateSchema = z.object({
  ...
  if (!value.reportingEligible && detailed.some((count) => count !== 0)) {
    ctx.addIssue({ code: "custom", message: "Suppressed cohorts must expose zero detailed counts." });
  }
  if (value.reportingEligible) {
    if (value.cohortMemberCount < KHPOS_REPORTING_MINIMUM) {
      ctx.addIssue({ code: "custom", message: "Reporting requires at least five active cohort members." });
    }
    if (value.activeProfileCount > value.cohortMemberCount || value.pathSelectedCount > value.activeProfileCount) {
      ctx.addIssue({ code: "custom", message: "Potential-direction counters are inconsistent." });
    }
    if (value.questParticipantCount > value.cohortMemberCount || value.evidenceBackedQuestParticipantCount > value.questParticipantCount) {
      ctx.addIssue({ code: "custom", message: "Capability-practice counters are inconsistent." });
    }
    if (value.projectParticipantCount > value.cohortMemberCount || value.projectCompletionParticipantCount > value.projectParticipantCount) {
      ctx.addIssue({ code: "custom", message: "Value-creation counters are inconsistent." });
    }
    if (value.continuationEligibleCount > value.cohortMemberCount || value.continuingCycleParticipantCount > value.continuationEligibleCount) {
      ctx.addIssue({ code: "custom", message: "Development-continuity counters are inconsistent." });
    }
  }
});
